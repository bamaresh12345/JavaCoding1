...
things:Change the array nums such that the first k elements of nums contain the unique elements in the
...
public class RemoveDuplicatesFromSortedArray1Leetcode26 {
    ...
    public static int removeSuplicatesFromSortedArray(int[] nums) {
        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            if (i < nums.length - 1 && nums[i] == nums[i + 1])  //* IMP * if i< nums.length-1 is not mentioned, ARRAYINDEX OUT OF BOND ERROR
            {
                continue;
            } else {
                nums[count++] = nums[i];
